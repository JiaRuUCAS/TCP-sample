/*
 * mperf, Copyright (c) 2014-2018, The Regents of the University of
...
void
mperf_err(struct mperf_test *test, const char *format, ...)
{
    va_list argp;
    char str[1000];
    ...
    if (test != NULL && test->json_output && test->json_top != NULL)
		...
		if (test && test->outfile && test->outfile != stdout) {
	    	fprintf(test->outfile, "mperf: %s\n", str);
		}
		else {
	    	fprintf(stderr, "mperf: %s\n", str);
		}
	}
    va_end(argp);
}
...
void
mperf_errexit(struct mperf_test *test, const char *format, ...)
{
    va_list argp;
    char str[1000];
